...
list_ftp_dir <- function(url)
{
    ...
    if (grepl("HTML listing generated by Squid", doc) == FALSE) {
        # Normal directory listing (text format)
        listing <- strsplit(doc, "\n", fixed=TRUE)[[1L]]
        ## Keep field no. 8 only
        pattern <- paste(c("^", rep.int("[^[:space:]]+[[:space:]]+", 8L)),
                         collapse="")
        listing <- sub(pattern, "", listing)
        listing <- sub("[[:space:]].*$", "", listing)
    } else {
        # Squid proxy directory listing (HTML format)
        m <- gregexpr("HREF=\"([^./][^/\"]*)", doc, perl=TRUE)
        l <- regmatches(doc, m)
        l2 <- unlist(lapply(l, function(x) sub("HREF=\\\"", "", x)))
        listing <- l2[c(TRUE, FALSE)] # select every other element
    }
    print(listing)
    listing
}
